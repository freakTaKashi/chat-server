...
function appendMessage(message) {
    ...
    // Добавляем класс 'sent' для сообщений от пользователя, 'received' для от админа
    newMessage.classList.add('message', message.role === 'user' ? 'sent' : 'received');

    // Создание аватара
    if (message.role === 'admin') {
        const avatar = document.createElement('div');
        avatar.classList.add('avatar');
        avatar.style.backgroundImage = 'url("avatars/admin_avatar.png")'; // Путь к аватару администратора
    ...
    messageFooter.innerText = `Message envoyé à ${formatTimestamp(message.timestamp)} | ${senderName}`;
...
async function updateChat() {
    try {
        const response = await fetch(`/api/messages/${chatId}?role=user`); // Используем относительный путь
        if (!response.ok) {
            const errorData = await response.json();
